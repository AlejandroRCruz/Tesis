    ...
    while cap.isOpened():
        ...
        for res in results:
            ...
            for pid in range(num_personas):
                ...
                # Condición: al menos un hombro Y una cadera > 0.90
                hombro_ok = (ls_c > 0.90) or (rs_c > 0.90)
                cadera_ok = (lh_c > 0.90) or (rh_c > 0.90)

                if hombro_ok and cadera_ok:
                    # Guardar en CSV
                    writer.writerow([
                        frame_idx, pid,
                        ls[0], ls[1], ls_c,
                    ...
                    # Mostrar en consola
                    print(f"Frame {frame_idx}, Persona {pid} -> CAPTURADO ✅ "
                          f"LS:{ls}({ls_c:.2f}), RS:{rs}({rs_c:.2f}), "
                          f"LH:{lh}({lh_c:.2f}), RH:{rh}({rh_c:.2f})")

        # Mostrar video en tiempo real (opcional)
