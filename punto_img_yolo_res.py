...
csv_file = open("resultados_pose_1280.csv", "w", newline="")
...
while cap.isOpened():
    ...
    # ----------- INICIO cronómetro -----------
    t0 = time.time()
    results = model(frame, imgsz=1280, conf=0.6, iou=0.45, classes=[0], max_det=20)
    ...
    for r in results:
        ...
        for pid, (person_kp, person_conf) in enumerate(zip(kps, confs)):
            ...
            if person_conf[selected[0]] < 0.80 or person_conf[selected[1]] < 0.80:
                continue
